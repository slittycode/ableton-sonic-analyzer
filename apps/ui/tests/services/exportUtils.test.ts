...
describe('generateMarkdown', () => {
  ...
  it('markdown includes MixDoctor section when report provided', () => {
    const report: import('../../src/types').MixDoctorReport = {
      genreName: 'Techno',
      profileId: 'techno',
      advice: [{ band: 'Sub Bass', issue: 'optimal', message: 'Balanced.', diffDb: 0.2 }],
      dynamicsAdvice: { issue: 'optimal', message: 'Solid dynamic range.', actualCrest: 8 },
      plrAdvice: { issue: 'optimal', message: 'PLR 7.0 dB is within target.', actualPlr: 7 },
      loudnessAdvice: { issue: 'optimal', message: 'On target.', actualLufs: -7.9, truePeak: -0.2 },
      stereoAdvice: { correlation: 0.84, width: 0.69, monoCompatible: true, message: 'OK' },
      overallScore: 82,
    };
    const markdown = generateMarkdown(basePhase1, null, null, report);
    expect(markdown).toContain('## Mix Doctor');
    expect(markdown).toContain('Overall Score: 82/100');
  });

  it('markdown omits MixDoctor when report is null', () => {
    const markdown = generateMarkdown(basePhase1, null, null, null);
    expect(markdown).not.toContain('## Mix Doctor');
  });

  it('markdown includes PLR section when plrAdvice present', () => {
    const report: import('../../src/types').MixDoctorReport = {
      genreName: 'House',
      profileId: 'house',
      advice: [],
      dynamicsAdvice: { issue: 'optimal', message: 'OK', actualCrest: 8 },
      plrAdvice: { issue: 'too-crushed', message: 'PLR 4.0 dB is below target.', actualPlr: 4 },
      loudnessAdvice: undefined,
      stereoAdvice: undefined,
      overallScore: 70,
    };
    const markdown = generateMarkdown(basePhase1, null, null, report);
    expect(markdown).toContain('### PLR');
    expect(markdown).toContain('PLR: 4 dB');
  });

  it('markdown includes Dynamic Character when present', () => {
    const phase1WithDynChar = {
      ...basePhase1,
      dynamicCharacter: {
        dynamicComplexity: 0.72,
        loudnessVariation: 3.41,
        spectralFlatness: 0.18,
        logAttackTime: -1.23,
        attackTimeStdDev: 0.45,
      },
    };
    const markdown = generateMarkdown(phase1WithDynChar, null);
    expect(markdown).toContain('### Dynamic Character');
    expect(markdown).toContain('Dynamic Complexity');
    expect(markdown).toContain('Loudness Variation');
    expect(markdown).toContain('Spectral Flatness');
    expect(markdown).toContain('Log Attack Time');
    expect(markdown).toContain('Attack Time Std Dev');
  });
});
