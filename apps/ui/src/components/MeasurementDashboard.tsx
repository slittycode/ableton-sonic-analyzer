      ...
      <svg viewBox="0 0 120 120" className="w-[120px] h-[120px]">
        ...
        >
          <animateTransform
            ...
            dur="8s" from="0 60 60" to="360 60 60" repeatCount="indefinite"
          />
        </circle>
        ...
        <circle
          ...
          strokeDasharray="4 7" opacity="0.25"
        >
          <animateTransform
            attributeName="transform" type="rotate"
            dur="12s" from="360 60 60" to="0 60 60" repeatCount="indefinite"
          />
        </circle>
        {/* Breathing center dot */}
        <circle cx="60" cy="60" r="4" fill="#ff8800">
          <animate attributeName="r" values="3;5.5;3" dur={`${pulseDuration}s`} repeatCount="indefinite" />
          <animate attributeName="opacity" values="0.8;0.35;0.8" dur={`${pulseDuration}s`} repeatCount="indefinite" />
        </circle>
        {/* BPM text */}
        <text x="60" y="57" textAnchor="middle" fill="#fff" fontSize="22" fontWeight="800"
          fontFamily="'JetBrains Mono', monospace">{Math.round(bpm)}</text>
        <text x="60" y="70" textAnchor="middle" fill="#555" fontSize="8"
          fontFamily="'JetBrains Mono', monospace">BPM</text>
        {/* Source badge */}
        {bpmSource && (
          <text x="60" y="82" textAnchor="middle" fill="#00ff9d80" fontSize="6.5"
            fontFamily="'JetBrains Mono', monospace">
            {bpmSource === 'percival_ratio_corrected' ? '● corrected' :
              bpmSource === 'rhythm_extractor_confirmed' ? '● confirmed' : '● detected'}
          </text>
        )}
      </svg>
    </motion.div>
...
const COMPARATIVE_ZONES: Record<string, { color: string; zones: string[]; max: number; unit?: string }> = {
  groove: { color: '#ff8800', zones: ['tight', 'loose', 'swung', 'free'], max: 1 },
  stability: { color: '#a78bfa', zones: ['erratic', 'loose', 'steady', 'locked'], max: 100, unit: '%' },
  danceability: { color: '#fbbf24', zones: ['ambient', 'chill', 'groovy', 'peak'], max: 1 },
  onsetRate: { color: '#34d399', zones: ['sparse', 'moderate', 'dense', 'maximal'], max: 8, unit: '/sec' },
};
...
const ComparativeMetricTile = ({
  ...
  return (
    ...
    >
      <span className="text-[7px] font-mono uppercase tracking-wider block" style={{ color: `${cfg.color}80` }}>
        {metricKey === 'onsetRate' ? 'Onset Rate' : metricKey}
      </span>
      <div className="flex items-baseline gap-1 mt-1">
        <span className="text-[22px] font-display font-extrabold tabular-nums" style={{ color: cfg.color }}>
          {cfg.unit === '/sec' ? value.toFixed(1) : displayValue}
        </span>
        {cfg.unit === '/sec' && (
          <span className="text-[9px] font-mono" style={{ color: `${cfg.color}60` }}>/sec</span>
        )}
      </div>
      {/* Scale bar with position marker */}
      <div className="relative mt-2.5">
        <div className="flex h-[6px] rounded-[3px] overflow-hidden">
          {cfg.zones.map((_, i) => (
            <div
              ...
              className="flex-1"
              style={{
                background: `linear-gradient(90deg, ${cfg.color}${i === Math.floor(pct / 25) ? '30' : '12'}, ${cfg.color}${i === Math.floor(pct / 25) ? '40' : '18'})`,
              }}
            />
          ))}
        </div>
        ...
        <div className="flex justify-between mt-1">
          {cfg.zones.map((z) => (
            <span key={z} className="text-[6px] font-mono text-[#444]">{z}</span>
          ))}
        </div>
      </div>
    </motion.div>
...
const BeatSequencerGrid = ({
  ...
  const vel = pad(accentPattern.length === 4
    ? accentPattern.flatMap((v) => [v, 0, 0, 0])
    : accentPattern);
  ...
  return (
    <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-sm p-3">
      <div className="text-[8px] font-mono uppercase tracking-widest text-[#555] mb-2">Beat Pattern</div>
      <div className="grid gap-[3px]" style={{ gridTemplateColumns: `32px repeat(${cols}, 1fr)` }}>
        {/* Beat number headers */}
        <div />
        {Array.from({ length: 4 }, (_, i) => (
          <div key={i} className="text-[7px] font-mono text-[#444] text-center" style={{ gridColumn: 'span 4' }}>
            {i + 1}
          </div>
        ))}
        {/* Kick row */}
        ...
        {vel.map((v, i) => (
          <div
            ...
            style={{
              background: `linear-gradient(to top, #00ff9d${Math.round((v / maxVel) * 0.8 * 255).toString(16).padStart(2, '0')}, transparent)`,
            }}
          />
        ))}
      </div>
...
const AccentPatternBars = ({
  ...
  const maxVal = Math.max(...kickAccent.slice(0, beats), ...hihatAccent.slice(0, beats), 0.001);

  return (
    <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-sm p-3">
      <div className="text-[7px] font-mono uppercase tracking-widest text-[#555] mb-2">Accent Pattern · {beats} beats</div>
      ...
      <div className="flex justify-around mt-1">
        {Array.from({ length: beats }, (_, i) => (
          <span key={i} className="text-[7px] font-mono text-[#333]">{i + 1}</span>
        ))}
      </div>
    </div>
  );
...
const BeatEnergyWaveform = ({
  ...
  for (let i = 0; i < len; i++) {
    if (i < kickAccent.length) {
      bars.push({ value: kickAccent[i], color: '#ff4444', opacity: 0.3 + (kickAccent[i] / maxVal) * 0.7 });
    }
    if (i < hihatAccent.length) {
      bars.push({ value: hihatAccent[i], color: '#60a5fa', opacity: 0.3 + (hihatAccent[i] / maxVal) * 0.7 });
    }
  }
  ...
  return (
    <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-sm p-3">
      <div className="text-[7px] font-mono uppercase tracking-widest text-[#555] mb-2">Beat Energy</div>
      <svg viewBox={`0 0 ${svgW} 50`} className="w-full h-[50px]">
        {bars.map((b, i) => {
          ...
          return (
            <rect
              key={i} x={x} y={50 - h - 4} width={barW} height={Math.max(h, 2)}
              rx="2" fill={b.color} opacity={b.opacity}
            />
          );
        })}
      </svg>
...
const SidechainEnvelope = ({
  ...
  const strengthLabel = pumpingStrength >= 0.7 ? 'heavy' : pumpingStrength >= 0.4 ? 'moderate' : 'subtle';

  return (
    <div className="bg-[#141414] border border-[#1e1e1e] rounded-sm p-3">
      <div className="flex justify-between items-center mb-1.5">
        <span className="text-[7px] font-mono uppercase tracking-widest text-[#555]">Sidechain Envelope</span>
      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-[36px]">
        ...
        {/* Beat grid lines */}
        {[0, 4, 8, 12].map((pos) => (
          <line key={pos} x1={(pos / 15) * w} y1="0" x2={(pos / 15) * w} y2={h}
...
const GatingBadge = ({
  ...
  const rateLabel = gatingRate === 16 ? '16th' : gatingRate === 8 ? '8th' : gatingRate === 4 ? 'quarter' : `${gatingRate}`;
  return (
      ...
      {gatingRegularity != null && (
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <div className="w-12 h-[4px] bg-[#1a1a1a] rounded-sm overflow-hidden">
            <div className="h-full rounded-sm bg-[#fbbf24]" style={{ width: `${gatingRegularity * 100}%`, opacity: 0.7 }} />
          </div>
        </div>
      )}
    </div>
    ...
    <div>
      ...
      <div className="space-y-[2px]">
        {tiers.map((tier) => {
          ...
          return (
            <div key={tier.label} className="flex gap-[1px]" style={{ height: tier.size === 16 ? 10 : tier.size === 8 ? 8 : 6 }}>
              {Array.from({ length: segCount }, (_, i) => (
                ...
                >
                  <span className="font-mono" style={{ fontSize: 6, color: `${tier.color}50` }}>{tier.label}</span>
                </div>
              ))}
            </div>
          );
  ...
  return (
    <div className="space-y-4">
      ...
      <Section id="section-meas-rhythm" number={6} title="Rhythm & Groove">
        {/* Zone 1 — BPM Pulse + Comparative Metric Tiles */}
        <div className="flex gap-4 items-start">
          <BreathingBpmPulse bpm={phase1.bpm} bpmSource={phase1.bpmSource} />
          <div className="flex-1 grid grid-cols-2 gap-2">
            {phase1.rhythmDetail && (
              <>
                <ComparativeMetricTile metricKey="groove" value={phase1.rhythmDetail.grooveAmount} delay={0} />
                <ComparativeMetricTile
                  metricKey="stability"
                  value={(phase1.rhythmDetail.tempoStability ?? 1 - phase1.rhythmDetail.grooveAmount) * 100}
                  delay={0.08}
                />
              </>
            )}
            {phase1.danceability && (
              <ComparativeMetricTile metricKey="danceability" value={phase1.danceability.danceability} delay={0.16} />
            )}
            {phase1.rhythmDetail && (
              <ComparativeMetricTile metricKey="onsetRate" value={phase1.rhythmDetail.onsetRate} delay={0.24} />
            )}
          </div>
        </div>

        {/* Zone 2 — Beat Pattern (toggleable Grid / Energy) */}
        {phase1.grooveDetail && phase1.grooveDetail.kickAccent?.length > 0 && phase1.grooveDetail.hihatAccent?.length > 0 && (
        ...
        {(phase1.grooveDetail || phase1.beatsLoudness) && (
          <div className="border-t border-border pt-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              {phase1.grooveDetail && (
                <div className="bg-[#141414] border border-[#1e1e1e] rounded-sm p-3">
                  <div className="text-[7px] font-mono uppercase tracking-widest text-[#555] mb-2">Swing</div>
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={s.label}>
                        <div className="flex justify-between mb-1">
                          <span className="text-[7px] font-mono" style={{ color: `${s.color}80` }}>{s.label}</span>
        ...
        {/* Zone 4 — Sidechain & Effects */}
        {(phase1.sidechainDetail || (phase1.effectsDetail && phase1.effectsDetail.gatingDetected)) && (
          <div className="border-t border-border pt-3 space-y-2">
            <span className="text-[10px] font-mono uppercase tracking-wide text-text-secondary block">
              Sidechain & Effects
            </span>
            {phase1.sidechainDetail && phase1.sidechainDetail.envelopeShape && phase1.sidechainDetail.envelopeShape.length > 0 && (
              <SidechainEnvelope
            ...
            {phase1.sidechainDetail && !phase1.sidechainDetail.envelopeShape && (
              <div className="bg-[#141414] border border-[#1e1e1e] rounded-sm p-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <span className="text-[7px] font-mono text-[#555] block">Pumping Strength</span>
                    <span className="text-sm font-display font-bold text-text-primary">
        ...
        {/* DFA (supplementary metric) */}
        {phase1.danceability && (
          <div className="border-t border-border pt-3">
            <MetricRow label="DFA (Rhythmic Complexity)" value={formatNumber(phase1.danceability.dfa, 3)} />
          </div>
        )}
      </Section>
