...
- **Optional MT3 polyphonic transcription client.** Added `src/services/mt3Client.ts`. POSTs `/api/analysis-runs/{run_id}/mt3-transcriptions` and reads the additive `mt3` namespace from the run snapshot. Only fires when the user opts in at upload time (form field `mt3_mode='enabled'`); the deterministic Session Musician view remains the default and measurement stays authoritative.
- **Transcription pianoroll UI.** Added `src/services/transcriptionPianorollClient.ts` and the `TranscriptionPianoroll` / `TranscriptionPianorollBlock` components. Velocity-encoded canvas heatmap mounted in the Session Musician suite when `transcriptionDetail.noteCount > 0`. Grounded in the Phase 1 `bpm` + `timeSignature` so every cell traces back to a measurement.
- **Per-recommendation corpus-verification badge** (`GOAL.md` sub-goal 4). Added `src/services/recommendationVerification.ts`, the generated `src/data/recommendationVerification.ts` artifact (from `apps/backend/scripts/evaluate_recommendations.py --verification-artifact`), and `components/RecommendationVerificationBadge.tsx`. Renders the corpus confidence band on each recommendation card; hidden when the corpus has no scored fixtures so the surface degrades gracefully.
