...
| `beat_evaluation.py` + `beat_report_html.py` | Research-only beat/downbeat measurement gate. Compares the shipping kick-accent downbeat heuristic against the neural `beat_this` tracker on a labeled corpus; emits a JSON report and a standalone HTML render. Optional neural deps live in `requirements-eval.txt` (separate venv). Driven by `scripts/evaluate_beats.py` (+ `scripts/build_beat_manifest.py`). Does not touch the `analyze.py` stdout contract. |
| `loudness_rec_evaluation.py` | Eval/test-only reachability check for prescriptive loudness/dynamics advice — renders and re-measures audio to confirm the deterministic subset of a loudness recommendation (gain-to-target-LUFS, true-peak ceiling) is physically reachable. Must not be imported by `analyze.py` or `server.py`. Mirrors `TRUE_PEAK_OVER_LINEAR` from the UI's `loudnessGuardrails.ts`. Driven by `scripts/evaluate_loudness_recs.py`. |
