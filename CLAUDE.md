...
`AnalysisResults.tsx` is the large results surface, lazy-loaded via Suspense; `analysisResultsViewModel.ts` (same directory) holds its pure projection helpers. Manual vendor chunks in `vite.config.ts` control bundle splitting.
