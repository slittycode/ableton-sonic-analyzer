> **⚠️ INVALIDATED — 2026-03-26**
> This report was generated from hand-crafted cache stubs in
> `tests/ground_truth/cache/`, not from real audio analysis.
> All F1=1.0 results and threshold recommendations are artefacts
> of the stub data and must not be used.
> Thresholds reverted to original engineering defaults in
> `apps/backend/prompts/phase2_system.txt`.
> Re-run calibration only after real audio files are placed in
> `tests/ground_truth/tracks/` matching the track IDs in `labels.json`.

# Confidence Calibration Results

Generated by `scripts/calibrate_confidence.py`

Tracks analyzed: 10

---

## Pitch Confidence (Melody)

**Purpose:** Threshold for flagging melody as "draft only"

**Current threshold:** 0.15

### Tested Thresholds

| Threshold | Precision | Recall | F1 Score | Tracks |
|-----------|-----------|--------|----------|--------|
| 0.05 | 0.0000 | 0.0000 | 0.0000 | 10 |
| 0.10 | 1.0000 | 0.5000 | 0.6667 | 10 |
| 0.15 | 1.0000 | 0.7500 | 0.8571 | 10 |
| 0.20 | 1.0000 | 1.0000 | 1.0000 | 10 |
| 0.25 | 1.0000 | 1.0000 | 1.0000 | 10 |

**Recommended threshold: 0.20**
- F1 Score: 1.0000
- Precision: 1.0000
- Recall: 1.0000
- Change from current: +0.05

---

## Chord Strength

**Purpose:** Threshold for flagging chords as "approximate"

**Current threshold:** 0.7

### Tested Thresholds

| Threshold | Precision | Recall | F1 Score | Tracks |
|-----------|-----------|--------|----------|--------|
| 0.50 | 1.0000 | 1.0000 | 1.0000 | 10 |
| 0.60 | 1.0000 | 1.0000 | 1.0000 | 10 |
| 0.70 | 1.0000 | 1.0000 | 1.0000 | 10 |
| 0.80 | 0.6667 | 1.0000 | 0.8000 | 10 |
| 0.90 | 0.4000 | 1.0000 | 0.5714 | 10 |

**Recommended threshold: 0.50**
- F1 Score: 1.0000
- Precision: 1.0000
- Recall: 1.0000
- Change from current: -0.20

---

## Pumping Confidence (Sidechain)

**Purpose:** Threshold for sidechain detection confidence

**Current threshold:** 0.4

### Tested Thresholds

| Threshold | Precision | Recall | F1 Score | Tracks |
|-----------|-----------|--------|----------|--------|
| 0.20 | 0.8571 | 1.0000 | 0.9231 | 10 |
| 0.30 | 1.0000 | 1.0000 | 1.0000 | 10 |
| 0.40 | 1.0000 | 1.0000 | 1.0000 | 10 |
| 0.50 | 1.0000 | 1.0000 | 1.0000 | 10 |

**Recommended threshold: 0.30**
- F1 Score: 1.0000
- Precision: 1.0000
- Recall: 1.0000
- Change from current: -0.10

---

## Summary of Recommendations

| Metric | Current | Recommended | Change | Justification |
|--------|---------|-------------|--------|---------------|
| Pitch Confidence | 0.15 | 0.20 | +0.05 | F1=1.0000 |
| Chord Strength | 0.70 | 0.50 | -0.20 | F1=1.0000 |
| Pumping Confidence | 0.40 | 0.30 | -0.10 | F1=1.0000 |

## Justification

Thresholds are selected to maximize F1 score, which balances:
- **Precision:** Of tracks flagged as low confidence, how many were actually wrong?
- **Recall:** Of tracks that were actually wrong, how many were flagged?

Higher precision means fewer false alarms (flagging good tracks as bad).
Higher recall means fewer missed issues (bad tracks flagged as good).
F1 balances these concerns equally.

## Implementation Notes

To apply these thresholds:

1. Update the prompt in `apps/ui/src/services/geminiPhase2Client.ts`:
   - `pitchConfidence < X = melody is draft only`
   - `chordStrength < X = chords approximate`
   - `pumpingConfidence < X = do not assert sidechain`

2. Update UI components that use these thresholds

3. Document any genre-specific exceptions in genre_corpus.md
