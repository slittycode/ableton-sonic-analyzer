...
- `ui-overhaul-2026-06-21.md` — completion record for the `apps/ui` design-system overhaul (PRs #165–#174): typography unified on `--text-*`, a single `ui/` primitive layer (duplicate `MeasurementPrimitives` deleted), accessibility to WCAG AA, and a style-discipline CI guard. Records the two planned phases deliberately dropped as churn, and the deferred monolith restyle/split.
