...
> **Status: analysis snapshot, recommendations not tracked.** Data-driven point-in-time look (2026-03-26) at which Phase 1 fields show up in Gemini Phase 2 recommendations. The "remove unused fields" recommendations near the end of this doc are not on any active backlog and the file paths they target may no longer exist (e.g. the `geminiPhase2Client.ts` it mentions was removed when transport consolidated into [`analysisRunsClient.ts`](../apps/ui/src/services/analysisRunsClient.ts)). The field names in the tables below are also point-in-time and may not match the current v1 schema — some are stale (e.g. `spectralDetail.centroid` is now `spectralDetail.spectralCentroid`, and `rhythmDetail.beatInterval` no longer exists). Check [`apps/backend/JSON_SCHEMA.md`](../apps/backend/JSON_SCHEMA.md) for the authoritative field list; treat this doc as historical.
