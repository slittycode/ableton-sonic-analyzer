...
- **Genre-generalization accuracy program** (`plans/genre-generalization-program.md`; audits: [`audits/genre-coverage-2026-07-13.md`](audits/genre-coverage-2026-07-13.md), [`audits/genre-target-sourcing-2026-07-13.md`](audits/genre-target-sourcing-2026-07-13.md)): the accuracy gates and genre classifier previously proved out almost exclusively on steady four-on-the-floor techno/house at 96–140 BPM; this program widened them across UK bass/garage, halftime/trap/dubstep, hard dance/gabber, and ambient/electro material. Seven PRs (#210–#216):
  ...
  - **`bpmOctaveEvidence`** (full-mode only, additive): surfacing-only octave/ratio candidates for the shipped `bpm`, scored against kick and hat/snare inter-onset streams. Never overrides `bpm` (PURPOSE.md invariant #1); recovers the true tempo octave on 16/17 real-audio test cases where it was previously halved.
  - **Loudness-accent meter/downbeat detection**: `analyze_time_signature` now folds per-beat low-band loudness (with harmonic-fold collapse) alongside onset counts, fixing 3/4, 6/8, and 7/8 meters that previously always misread as 4/4 — `timeSignatureCandidates` gains an additive `loudnessDominance` field.
  - **16th-grid swing detection** (`compute_swing_detail`): probes the 16th-note grid for UK garage/2-step shuffle when the 8th-note grid reads straight — previously invisible to the measurement entirely.
  - Nine new/retuned `_GENRE_SIGNATURES` (gabber, hardstyle, electro, downtempo, breaks, dubstep, psytrance measured from 93 real GiantSteps reference clips; trap, footwork, two-step, speed-garage, halftime-dnb knowledge-based pending a licensed reference corpus), a widened `genreFamily` union (`garage`, `hardcore`, `trap`, `electro`, `downtempo`) mirrored across `_GENRE_FAMILY_MAP`, `apps/ui/src/types/measurement.ts`, and `backendPhase1Client.ts`, an octave-aware BPM axis in `analyze_genre_detail` (via `bpmOctaveEvidence`), and four measured mix-doctor profiles in [`genreProfiles.ts`](apps/ui/src/data/genreProfiles.ts). Real-clip replay: correct genre-family top-1 classification went from ~0/93 to 51/93.
...
- **Research-only beat/downbeat measurement gate** ([`apps/backend/beat_evaluation.py`](apps/backend/beat_evaluation.py) + `beat_report_html.py`, `scripts/evaluate_beats.py`, `scripts/build_beat_manifest.py`): benchmarks CPJKU/beat_this against the shipping kick-accent downbeat heuristic on a labeled corpus. Off the product path; deleting it restores the product exactly.
- **Loudness-recommendation reachability eval** ([`apps/backend/loudness_rec_evaluation.py`](apps/backend/loudness_rec_evaluation.py) + `scripts/evaluate_loudness_recs.py`): eval/test-only verification that the deterministic subset of a loudness recommendation (gain-to-target-LUFS + true-peak ceiling) is physically reachable. Not imported by `analyze.py` or `server.py`.
- **Phase 1 golden-snapshot regression gate** ([`apps/backend/tests/test_phase1_golden.py`](apps/backend/tests/test_phase1_golden.py), fixture `tests/fixtures/golden/phase1_default.json`): pins measured Phase 1 values so unintended numeric drift fails CI.
...
### Fixed
- **Host-independent audio MIME resolution**: new [`apps/backend/audio_mime.py`](apps/backend/audio_mime.py) pins the filename→MIME map (mirroring `apps/ui/src/services/audioFile.ts`) so a `.flac` resolves to `audio/flac` on every OS instead of varying by host (`audio/x-flac` on macOS vs `audio/flac` on Linux) — the divergence failed the backend test gate and could mislabel a FLAC handed to Gemini.
